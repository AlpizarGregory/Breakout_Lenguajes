//
// Created by rachel10 on 12/06/22.
//
/**
...
int main ()
{
    ...
    if(socketServidor == -1)
    {
        perror("Error al abrir socket servidor, verifique si el puerto esta disponible");
        exit(-1);
    }
        ...
        for(i = 0; i < numeroClientes; i++)
        {
            if(FD_ISSET (socketCliente[i], &descriptoresLectura))
            {
                ...
                if((Lee_Socket(socketCliente[i], (char *)&buffer, sizeof(int)) > 0))
                {
                    ...
                    int longdatosCliente = strlen(parseJson())+1;

                    auxiliar = htonl(longdatosCliente);

                    ///Se envia el entero transformado
                    Escribe_Socket(socketCliente[i], (char *)&auxiliar, sizeof(int));
                    //printf("Servidor C: Envia %d \n", longdatosCliente-1);

                    ///Se envia la cadena recibida
                    Escribe_Socket(socketCliente[i], parseJson(), longdatosCliente);
                    //printf("Servidor C: Envia %s \n", parseJson());
